...
from monai.data import list_data_collate, decollate_batch, DataLoader, Dataset, TestTimeAugmentation
import torch
...
class Net(pytorch_lightning.LightningModule):
    ...
    def prepare_data(self, data_dir):
        ...
        # data_dir = '/home/navid/Desktop/Papers/MICCAI_challenge_1/nifti/FDG-PET-CT-Lesions'
...
def segment_PETCT_dyn(ckpt_path, data_dir):
    print("starting")
    model = Net()
    ...
    post_transforms = Compose(
        [
            Activationsd(keys="PRED_dyn", sigmoid=True),
            Invertd(
                keys="PRED_dyn",  # invert the `pred` data field, also support multiple fields
                transform=model.val_transforms,
                orig_keys="image_ct",  # get the previously applied pre_transforms information on the `img` data field,
            ...
            AsDiscreted(keys="PRED_dyn", argmax=True, to_onehot=2)
        ]
    )
    ...
    #         sw_batch_size = 4
    #
    #         val_data["PRED_dyn"] = sliding_window_inference(val_data["image_petct"].to(device), roi_size, sw_batch_size,
    #                                                         model)
            # val_data = [post_transforms(i) for i in decollate_batch(val_data)]
            # #
            # mask_out = torch.argmax(val_data[0]["PRED_dyn"], dim=0).detach()
            ...
            # print(os.path.join(data_dir, "PRED_dyn.nii.gz"))
            #
            # nib.save(mask_export, os.path.join(data_dir, "PRED_dyn.nii.gz"))
            # print("done writing")
            # return val_data["PRED_dyn"]


def dyn_run_inference(ckpt_path='/opt/algorithm/best_dyn_metric_model.pth', data_dir='/opt/algorithm/'):
    return segment_PETCT_dyn(ckpt_path, data_dir)


if __name__ == '__main__':
    pred_dyn = dyn_run_inference()
